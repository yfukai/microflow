...
STRATEGIES = ["percentile", "mean", "pybaselines"]
PYBASELINES_METHODS = ["imodpoly"]
...
class Config:
    ...
    pybaselines_method : str = "imodpoly" # options: "imodpoly"
...
def main():
    ...
    if cfg.strategy not in STRATEGIES:
        raise ValueError(f"Invalid strategy: {cfg.strategy}. Must be one of {STRATEGIES}")
    if cfg.strategy == "pybaselines" and cfg.pybaselines_method not in PYBASELINES_METHODS:
        raise ValueError(f"Invalid pybaselines method: {cfg.pybaselines_method}. Must be one of {PYBASELINES_METHODS}")
